...
FileNode* FAT32::FindFileByPath(const char* path)
{
	...
	if (last_index < strlen(path)) //最后一个section
	{
...
int FAT32::Read(FileNode* file_node, char* buffer, int size)
{
	...
	Uint64 total_has_read_size = 0;//可能要跨簇读取，这是这次READ多个簇读取的总数据量
	...
	Uint64 lba = GetLbaFromCluster(node->CurCluster) + (node->ReadSize % bytes_per_cluster) / SECTIONSIZE;//当前簇LBA
	...
	while (size)
	{
		...
		Uint64 section_has_read = node->ReadSize % (SECTIONSIZE);//当前扇区已读字节
		...
		if (node->ReadSize % SECTIONSIZE == 0)//这个扇区已经读完
		{
			lba++;
		}
		if (node->ReadSize % bytes_per_cluster == 0) //这个簇已经读完
		{
			node->CurCluster = GetFATContentFromCluster(node->CurCluster);
			lba = GetLbaFromCluster(node->CurCluster);//新簇LBA
		}
	}
